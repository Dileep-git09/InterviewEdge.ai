  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      toast(
        (t) => (
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2 mt-1">
              <button
                onClick={() => { toast.dismiss(t.id); navigate(`/resume-prep/${sessionId}`); }}
        ...
        { duration: 12000, style: { maxWidth: "320px" } }
      );
    } catch (error) {
      toast.dismiss(loadingToastId);
      toast.error(
        error?.response?.data?.message || "Failed to analyse resume. Please try again."
      );
    } finally {
      ...
      {isOpen && (
        <div className="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-[200]">
          ...
          <div className="py-2">
            ...
            />
            {/* ✅ My Profile is now active — navigates to /profile */}
            <MenuItem
              icon={<LuUser size={16} />}
              label="My Profile"
